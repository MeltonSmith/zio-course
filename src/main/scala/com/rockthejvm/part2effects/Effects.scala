...
import com.rockthejvm.part2effects.Effects.Scala
...
object Effects {
  ...
  //1
  val timeIo: MyIO[Long] = MyIO[Long](() => System.currentTimeMillis())
  ...
  def measure[A](computation: MyIO[A]): MyIO[(Long, A)] = {
      for {
        ...
        result <- computation
        timeEnd <- timeIo
      } yield (timeEnd - timeStart, result)
  ...
  def chill[A, Sober](dev: A): Result = {
    Right("Relax")
  }
  ...
  type Sober = String
  type Relax = String
  type Result = Either[Sober, Relax]

  case class VacationIO[-Dev, A](consume: Dev => A) {
    def map[B](dev: Dev)(f: A => B): VacationIO[Dev, B] = {
      VacationIO[Dev, B](dev => f(consume(dev)))
    }

    def flatMap[BDev <: Dev, B](f: A => VacationIO[BDev, B]): VacationIO[BDev, B] = {
      VacationIO[BDev, B](dev => f(consume(dev)).consume(dev))
    }
